...
class IntermediateCollect(SubModularSpark[T]):
    ...
    def run(self) -> set[T]:
        ...
        rdd: pyspark.RDD[tuple[T, int]] = self.parse_func(self.file, self.sc, 1)
        n: int = rdd.count()
        ...
        while L <= self.k:
            reducer3: list[tuple[int, list[T]]] = (
                self.sc.parallelize(mapper1, 1)
                .reduceByKey(
                    lambda a, b:
            ...
            mapper4: list[tuple[int, list[T]]] = (
                self.sc.parallelize(reducer3, 1)
                .map(lambda pair: (0, pair[1]))
            ).collect()

            reducer4: list[tuple[int, list[T | int] | T]] = (
                self.sc.parallelize(mapper4, 1)
                .reduceByKey(
                    lambda a, b:
