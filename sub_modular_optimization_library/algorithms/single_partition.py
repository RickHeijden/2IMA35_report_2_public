...
class SinglePartition(SubModularSpark[T]):
    ...
    def run(self) -> set[T]:
        ...
        rdd: pyspark.RDD[tuple[T, int]] = self.parse_func(self.file, self.sc, 1, True)
        n: int = rdd.count()
        ...
        mapper1: pyspark.RDD[tuple[int, T]] = (
            rdd
            .map(lambda pair: (math.ceil((int(pair[1]) + 1) / math.sqrt(n)), pair[0]), True)
        )

        reducer1: pyspark.RDD[tuple[int, tuple[T, float]]] = (
            mapper1
            .map(lambda a: (a[0], (a[1], use_func(broadcast_func)({a[1]}))), True)
            .reduceByKey(lambda a, b: a if a[1] >= b[1] else b, 1)
        )
